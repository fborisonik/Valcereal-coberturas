# ============================================================
#   VALCEREAL — Coberturas  v4.0
#   Analizador de Derivados Agro · Streamlit Web App
#   pages/1_Coberturas.py
# ============================================================
# Cambios v4.0:
#   - Futuros: columna Compra/Venta + P&L impacto en gráfico
#   - Borrado de filas: botón ✕ nativo del data_editor
#   - Validación: permite calcular solo con futuros, solo con
#     opciones, o combinado
#   - Info panel y análisis actualizados para futuros
# ============================================================
...
# ── Page config ──────────────────────────────────────────────
st.set_page_config(
    page_title="Coberturas — Valcereal",
    page_icon="🌱",
    layout="wide",
...
# Paleta para múltiples operaciones (futuros + opciones)
COLORES_OPS = [VC_VERDE_OSC, VC_AZUL, "#8E44AD", "#E67E22", "#E74C3C", "#17A589"]
...
with st.sidebar:
    ...
    **Cómo usar:**
    ...
    2. Agregá futuros (Compra o Venta)
    3. Cargá las opciones
    4. Usá el **✕** en cada fila para borrar una operación
    5. Hacé clic en **Calcular posición**
    """)
    st.markdown("---")
...
<div style="background-color:{VC_VERDE_OSC}; padding:1.1rem 1.8rem;
  ...
  <span style="font-size:1rem; color:#AADDCC; margin-left:1rem;">
    Coberturas — Analizador de Derivados Agro
  </span>
</div>
""", unsafe_allow_html=True)
...
# ════════════════════════════════════════════════════════════
# SECCIÓN 2 — Futuros (tabla dinámica con Compra/Venta)
# ════════════════════════════════════════════════════════════
st.markdown('<div class="seccion">📈 &nbsp; Futuros</div>', unsafe_allow_html=True)
st.markdown(
    '<p class="hint">Compra = long (apostás a suba) &nbsp;·&nbsp; '
    'Venta = short (cobertura típica del productor). '
    'Usá ✕ para borrar filas.</p>',
    unsafe_allow_html=True
)
...
futuros_df = st.data_editor(
    ...
    column_config={
        ...
        "Posición": st.column_config.SelectboxColumn(
            ...
            required=True,
            width="small",
            help="Venta: short (fija precio venta). Compra: long (fija precio compra).",
        ),
    ...
    hide_index=True,
    column_order=["Mes", "Posición", "Precio (USD/tn)"],
)
...
st.markdown('<div class="seccion">⚙️ &nbsp; Opciones &nbsp;'
            ...
            unsafe_allow_html=True)
st.markdown(
    '<p class="hint">Podés cargar múltiples opciones. Usá ✕ para borrar filas.</p>',
    unsafe_allow_html=True
)
...
if calcular:
    ops_validas  = opciones_df.dropna(
        subset=["Tipo", "Posición", "Strike (USD/tn)", "Prima (USD/tn)"])
    futs_validos = futuros_df.dropna(
        subset=["Mes", "Posición", "Precio (USD/tn)"])

    if len(ops_validas) == 0 and len(futs_validos) == 0:
        st.error("⚠  Ingresá al menos un futuro o una opción para calcular.")
        st.stop()
    ...
    st.session_state["snapshot"] = {
        ...
        "futuros":     futs_validos.to_dict("records"),
...
primer_mes = futuros[0]["Mes"] if futuros else (
    "—" if not opciones else "—"
)
año_actual = date.today().year

# ── Rango de precios ─────────────────────────────────────────
refs = [PRECIO_SPOT]
refs += [f["Precio (USD/tn)"] for f in futuros]
refs += [o["Strike (USD/tn)"]  for o in opciones]
precio_min = min(refs) * 0.78
precio_max = max(refs) * 1.22
...
# ── Futuros ──────────────────────────────────────────────────
fut_lines = []   # (y_arr, label, color, posicion, precio_fut, mes)

for i, fut in enumerate(futuros):
    pf  = float(fut["Precio (USD/tn)"])
    pos = str(fut.get("Posición", "Venta")).lower()
    mes = fut["Mes"]
    color = COLORES_OPS[i % len(COLORES_OPS)]

    if pos == "venta":
        # Short futuro: productor fija precio de venta
        # Gana cuando precio cae por debajo del futuro, pierde cuando sube
        y_fut = pf - precios
        label = f"Fut. Venta {mes}  ${pf:.0f}/tn"
    else:
        # Long futuro: fija precio de compra
        # Gana cuando precio sube por encima del futuro
        y_fut = precios - pf
        label = f"Fut. Compra {mes}  ${pf:.0f}/tn"

    y_combined += y_fut
    fut_lines.append((y_fut, label, color, pos, pf, mes))

# ── Opciones ─────────────────────────────────────────────────
op_lines = []   # (y_arr, label, color, tipo, posicion, K, P, clave_txt)
...
for i, op in enumerate(opciones):
    ...
    color    = COLORES_OPS[(len(fut_lines) + i) % len(COLORES_OPS)]
...
prima_neta = sum(
    (-o["Prima (USD/tn)"] if str(o["Posición"]).lower() == "compra"
     else o["Prima (USD/tn)"])
    for o in opciones
)

# Breakevens (cruces con cero)
zero_crossings = []
...
be_str = "  /  ".join(f"${x:.1f}" for x in zero_crossings) if zero_crossings \
         else "No hay en el rango"
...
for fut in futuros:
    ...
    if pos == "venta":
        partes.append(
            f"vendió un futuro de {PRODUCTO} {mes} a ${pf:.0f}/tn "
            f"(precio de venta fijo; gana si el mercado cae, pierde el upside si sube)"
        )
    else:
        partes.append(
            f"compró un futuro de {PRODUCTO} {mes} a ${pf:.0f}/tn "
            f"(precio de compra fijo; gana si el mercado sube, pierde si cae)"
        )
...
for op in opciones:
    ...
    if tipo == "put" and pos == "compra":
        partes.append(f"compró Put K=${K:.0f}/tn prima -${P}/tn → piso ${K - P:.1f}/tn")
    elif tipo == "put" and pos == "venta":
        partes.append(f"vendió Put K=${K:.0f}/tn prima +${P}/tn → BE ${K - P:.1f}/tn")
    elif tipo == "call" and pos == "compra":
        partes.append(f"compró Call K=${K:.0f}/tn prima -${P}/tn → BE ${K + P:.1f}/tn")
    else:
        partes.append(f"vendió Call K=${K:.0f}/tn prima +${P}/tn → cap ${K + P:.1f}/tn")

be_display = be_str if zero_crossings else "no hay breakeven en el rango simulado"
ops_txt    = "  ·  ".join(partes)
prima_txt  = (
    f"  Prima neta opciones: {'−' if prima_neta < 0 else '+'}${abs(prima_neta):.2f}/tn."
    if opciones else ""
)
explicacion = (
    f"{CLIENTE} {ops_txt}.{prima_txt}  "
    f"P&L máximo en el rango: +${max_gain:.1f}/tn / −${abs(max_loss):.1f}/tn.  "
...
col_a.metric("Producto",   PRODUCTO)
col_b.metric("Futuros",    f"{n_fut} operación{'es' if n_fut != 1 else ''}")
col_c.metric("Opciones",   f"{n_ops} operación{'es' if n_ops != 1 else ''}")
col_d.metric("Prima neta", f"{'−' if prima_neta < 0 else '+'}${abs(prima_neta):.2f}/tn"
             if opciones else "—")
col_e.metric("Breakeven",
             f"${zero_crossings[0]:.1f}/tn" if zero_crossings else "—")
...
def generar_figura():
    ...
    ax_chart = fig.add_axes([0.06, 0.28, 0.56, 0.54])
    ...
    # Curvas individuales (si hay más de 1 operación total)
    if n_total > 1:
        for y_fut, label, color, *_ in fut_lines:
            ax.plot(precios, y_fut, color=color, linewidth=1.5,
                    linestyle=':', alpha=0.70, label=label, zorder=3)
        for y_op, label, color, *_ in op_lines:
            ax.plot(precios, y_op, color=color, linewidth=1.5,
                    linestyle=':', alpha=0.70, label=label, zorder=3)

    # Curva combinada
    if n_total > 1:
        lbl_comb   = "P&L combinado"
        color_comb = VC_VERDE_OSC
    elif fut_lines:
        lbl_comb   = fut_lines[0][1]
        color_comb = fut_lines[0][2]
    else:
        lbl_comb   = op_lines[0][1]
        color_comb = op_lines[0][2]

    ax.plot(precios, y_combined, color=color_comb, linewidth=2.8,
            linestyle='-', label=lbl_comb, zorder=4)
    ...
    # Anotaciones futuros
    _rng = y_combined.max() - y_combined.min() or 1.0
    for fut, (y_fut, label, color, pos, pf, mes) in zip(futuros, fut_lines):
        ax.axvline(pf, color=color, linewidth=1.2,
                   linestyle=':', alpha=0.55, zorder=2)
        idx  = np.argmin(np.abs(precios - pf))
        ypos = y_combined[idx]
        pos_lbl = "Vta." if pos == "venta" else "Cmp."
        ax.annotate(
            f"Fut. {pos_lbl} {mes}\n${pf:.0f}/tn",
            xy=(pf, ypos),
            xytext=(pf - (precio_max - precio_min) * 0.14,
                    ypos + _rng * 0.12),
            fontsize=7.5, fontweight='bold', color=color,
            arrowprops=dict(arrowstyle='->', color=color, lw=1.0),
            bbox=dict(boxstyle='round,pad=0.25', fc=VC_BLANCO,
    ...
    ax.annotate(
        ...
        xytext=(PRECIO_SPOT + (precio_max - precio_min) * 0.05,
                y_spot - _rng * 0.18),
        fontsize=7.5, fontweight='bold', color=VC_AMARILLO,
        arrowprops=dict(arrowstyle='->', color=VC_AMARILLO, lw=1.0),
        bbox=dict(boxstyle='round,pad=0.25', fc=VC_BLANCO,
                  ec=VC_AMARILLO, alpha=0.92, lw=0.9),
    )

    # Breakevens
    for xbe in zero_crossings[:2]:
        ax.axvline(xbe, color=VC_GRIS_LIN, linewidth=0.9,
        ...
        ax.annotate(
            ...
            bbox=dict(boxstyle='round,pad=0.2', fc=VC_BLANCO,
                      ec=VC_GRIS_LIN, alpha=0.85, lw=0.8),
        )

    ylabel = ("P&L Posición en Derivados (USD/tn)" if fut_lines
              else "P&L Opciones (USD/tn)")
    ax.set_xlabel(f"Precio {PRODUCTO} {primer_mes} (USD/tn)",
                  fontsize=10, color=VC_NEGRO, labelpad=10)
    ax.set_ylabel(ylabel, fontsize=10, color=VC_NEGRO, labelpad=8)
    ...
    metricas.append(("─" * 20, "─" * 8))

    # Futuros
    for fut in futuros[:4]:
        pf  = float(fut["Precio (USD/tn)"])
        pos = str(fut.get("Posición", "Venta"))
        metricas.append((
            f"Fut {pos[:3].upper()}. {fut['Mes'][:3]}.",
            f"${pf:.2f}/tn"
        ))
    ...
    # ── Header ────────────────────────────────────────────────
    if n_total > 1:
        titulo_op = f"Estrategia de Cobertura — {PRODUCTO} {primer_mes} {año_actual}"
    elif fut_lines:
        titulo_op = f"{fut_lines[0][1]} — {PRODUCTO} {primer_mes} {año_actual}"
    else:
        titulo_op = (
            f"{op_lines[0][1].split('K=')[0].strip()} "
...
with col_desc:
    fut_resumen = "  |  ".join(
        f"Fut {str(f.get('Posición','Vta'))[:3].upper()}. {f['Mes'][:3]} ${float(f['Precio (USD/tn)']):.0f}"
        for f in futuros
    )
    ops_resumen = "  |  ".join(
    ...
    resumen_parts = [p for p in [fut_resumen, ops_resumen] if p]
    st.markdown(
        f"**{CLIENTE}** &nbsp;·&nbsp; {PRODUCTO} {primer_mes}"
        f"{'  &nbsp;·&nbsp;  ' + '  '.join(resumen_parts) if resumen_parts else ''}  \n"
